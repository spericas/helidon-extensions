 ...
 */
@Service.Singleton
class Kv1SecretsFactory implements Supplier<Optional<Kv1Secrets>> {
    private final Optional<Vault> vault;

    Kv1SecretsFactory(Optional<Vault> vault) {
        this.vault = vault;
    }

    @Override
    public Optional<Kv1Secrets> get() {
        return vault.map(it -> it.secrets(Kv1Secrets.ENGINE));
    }
}
