 ...
 */
@Service.Singleton
class TransitSecretsFactory implements Supplier<Optional<TransitSecrets>> {
    private final Optional<Vault> vault;

    TransitSecretsFactory(Optional<Vault> vault) {
        this.vault = vault;
    }

    @Override
    public Optional<TransitSecrets> get() {
        return vault.map(it -> it.secrets(TransitSecrets.ENGINE));
    }
}
