 ...
 */
@Service.Singleton
class Kv2SecretsFactory implements Supplier<Optional<Kv2Secrets>> {
    private final Optional<Vault> vault;

    Kv2SecretsFactory(Optional<Vault> vault) {
        this.vault = vault;
    }

    @Override
    public Optional<Kv2Secrets> get() {
        return vault.map(it -> it.secrets(Kv2Secrets.ENGINE));
    }
}
