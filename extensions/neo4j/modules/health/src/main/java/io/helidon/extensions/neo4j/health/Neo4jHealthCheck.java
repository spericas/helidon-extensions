 ...
 */
package io.helidon.extensions.neo4j.health;
    ...
    public HealthCheckResponse call() {
        ...
        try (Session session = this.driver.session()) {
            return session.executeWrite(tx -> {
                ...
                if (databaseInfo.name() != null && !databaseInfo.name().trim().isBlank()) {
            ...
            return builder.status(HealthCheckResponse.Status.DOWN)
