...
HashiCorp Vault is a commonly used Vault in many microservices. The APIs are REST-based and Helidon implements them
using [WebClient]({webclient-page}).

### Maven Coordinates

To enable Vault integration, add the following dependency to your project's `pom.xml` file.
...
Each of these features is implemented as a separate module, with the Vault class binding them together. Code to set up Vault and
obtain a specific secret engine:
...
* AppRole authentication - AppRole ID and secret ID are configured, integration exchanges these for a temporary token that is used
  to connect to Vault
...
* K8s authentication - the k8s JWT token is discovered on current node and used to obtain a temporary token that is used to
  connect to Vault
...
New secret engines and authentication methods can be implemented quite easily, as the integration is based on service providers (
using ServiceLoader). This gives us (or you, as the users) the option to add new secret engines and/or authentication methods
without adding a plethora of methods to the Vault class.

See the following SPIs:

```
io.helidon.extensions.hashicorp.vault.spi.AuthMethodProvider
io.helidon.extensions.hashicorp.vault.spi.SecretsEngineProvider
io.helidon.extensions.hashicorp.vault.spi.SysProvider
io.helidon.extensions.hashicorp.vault.spi.VaultAuth
io.helidon.extensions.hashicorp.vault.spi.InjectionProvider
```
