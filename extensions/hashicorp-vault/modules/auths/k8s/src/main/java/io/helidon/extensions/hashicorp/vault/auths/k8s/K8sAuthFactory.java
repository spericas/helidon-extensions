 ...
 */
@Service.Singleton
class K8sAuthFactory implements Supplier<Optional<K8sAuth>> {
    private final Optional<Vault> vault;

    K8sAuthFactory(Optional<Vault> vault) {
        this.vault = vault;
    }

    @Override
    public Optional<K8sAuth> get() {
        return vault.map(it -> it.auth(K8sAuth.AUTH_METHOD));
    }
}
