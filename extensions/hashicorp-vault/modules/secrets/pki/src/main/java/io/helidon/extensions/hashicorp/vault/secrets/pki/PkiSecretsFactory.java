 ...
 */
@Service.Singleton
class PkiSecretsFactory implements Supplier<Optional<PkiSecrets>> {
    private final Optional<Vault> vault;

    PkiSecretsFactory(Optional<Vault> vault) {
        this.vault = vault;
    }

    @Override
    public Optional<PkiSecrets> get() {
        return vault.map(it -> it.secrets(PkiSecrets.ENGINE));
    }
}
